 ...
 */
function randomRotate(v, maxAngle) {
  let axis = new THREE.Vector3(
    Math.random() - 0.5,
    Math.random() - 0.5,
    Math.random() - 0.5
  ).normalize();
  if (Math.abs(axis.dot(v)) > 0.99) {
  ...
  return v.clone().applyQuaternion(quat).normalize();
}
...
function generatePartialPoints(numPoints, maxAngle, biasAngle, minStep, maxStep, roadWidth, buffer) {
  ...
  for (let i = 1; i < finalCount; i++) {
    ...
    let newDir;
    if (i < finalCount / 2) {
      newDir = randomRotate(direction, maxAngle);
    } else {
      const toStart = start.clone().sub(current).normalize();
      newDir = direction.clone().multiplyScalar(0.7).add(toStart.multiplyScalar(0.3)).normalize();
      newDir = randomRotate(newDir, biasAngle);
    }
    do {
...
function generatePointsBetween(pStart, pEnd, numPoints, maxAngle, biasAngle, minStep, maxStep, roadWidth, buffer) {
  ...
  for (let i = 1; i < finalCount; i++) {
    ...
    if (i < finalCount / 2) {
      ...
      newDir = direction.clone().multiplyScalar(0.7).add(toEnd.multiplyScalar(0.3)).normalize();
      newDir = randomRotate(newDir, biasAngle);
    }
    do {
      const step = THREE.MathUtils.lerp(minStep, maxStep, Math.random());
